...
public class ETLPreviewLoad {
	...
	WatchDog.log(WatchDog.WATCHDOG_ENV_PREV, "previewload", "Acquired lock and changed process state to PREVIEW_DELTA_LOAD_IN_PROGRESS", WatchDog.WATCHDOG_INFO);
	WatchDog.log(WatchDog.WATCHDOG_ENV_PREV, "previewload", "Changed internal process state to PREVIEW_XML_IN_PROGRESS", WatchDog.WATCHDOG_DEBUG);
...
**/
	...
	ProcessState.setLock(ProcessState.LOCK_CLEAR);
	WatchDog.log(WatchDog.WATCHDOG_ENV_PREV, "previewload", "Released lock and changed process state to PREVIEW_DELTA_LOAD_COMPLETE", WatchDog.WATCHDOG_INFO);
	pu.cleanup();
    }
	...
	for (int i = 0, size = element.nodeCount(); i < size; i++) {
		...
		if (e.getName().equals(entity)) {
		    ...
		    System.out.println(entity + " PK = " + Entity.getPrimaryKey(e, entity));
