...
public class ETLProductionLoad {
	...
	WatchDog.log(WatchDog.WATCHDOG_ENV_PROD, "prodload", "Acquired lock and changed process state to PROD_DELTA_LOAD_IN_PROGRESS", WatchDog.WATCHDOG_INFO);
	ProductionLoad pl = new ProductionLoad();
	pl.init();
	pl.run();
	ProcessState.setLock(ProcessState.LOCK_CLEAR);
	ProcessState.setSystemState(ProcessState.PROD_DELTA_LOAD_COMPLETE);
	WatchDog.log(WatchDog.WATCHDOG_ENV_PROD, "prodload", "Released lock and changed process state to PROD_DELTA_LOAD_COMPLETE", WatchDog.WATCHDOG_INFO);
	pl.cleanup();
    }
